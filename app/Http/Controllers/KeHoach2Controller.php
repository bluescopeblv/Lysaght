...
class KeHoach2Controller extends Controller
{
    ...
    {   
        ...
        $chitietCO = KeHoach::where('CO',$CO)
                            ...
                            ->where('Litem',$LItem)
                            ->where('Plan','like',"OK")
                            ->where('DateSX_KH_DMY',$datesx)
                            ->where('ThuTuCO',$ThuTuCO)
                            ->orderBy('Priority1')
                            ->orderBy('ChieuDai','desc')->orderBy('MO')->paginate(15);
        //$loaitin = LoaiTin::find($id);
        //$tintuc = TinTuc::where('idLoaiTin',$id)->paginate(5);
        //echo "string ".$chitietCO;
        return view('pages.chitiet',['chitietCO'=>$chitietCO,'CO'=>$CO,'Litem'=>$LItem, 'wc'=>$wc, 'DateSX_KH_DMY'=>$datesx, 'ThuTuCO' => $ThuTuCO]);
    }


    function getReportAll($CO,$LItem,$wc,$datesx,$ThuTuCO)
    {   
