    ...
    <div class="col-md-12">
        <div class="panel panel-info">
            <div class="panel-body">
                <span class="tieude">UPDATE KPI - MAINTENANCE</span>
                ...
                <table id="myTable" class="table table-bordered table-striped color-bordered-table info-bordered-table hover-table">
                    <thead>
                        <tr>
                            <th>#</th>
                            <th>Thời gian</th>
                            <th>No of Break down time</th>
                            <th>Lead time of fixing (Min/breakdown)</th>
                            <th>Preventive Maintenance (%)</th>
                            <th>Maintenance cost (Mil)</th>
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td>{{ $val->id }}</td>
                            <td>{{ $val->name }}</td>
                            <td>{{ number_format($val->MAI_BREAK_NUMBER, 0)  }}</td>
                            <td>{{ number_format($val->MAI_BREAK_LEADTIME, 2) }}</td>
                            <td>{{ number_format($val->MAI_PERCEN_PREVENTIVE, 2) }}</td>
                            <td>{{ number_format($val->MAI_COST, 0) }}</td>
                            
                            <td>
                                <span class="label label-warning">
                                    <a href="kpi/maintenance/edit/{{$val->id}}"><span class="glyphicon glyphicon-edit">Sửa</span></a>
                                </span><span>  </span>
